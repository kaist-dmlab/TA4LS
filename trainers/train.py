    ...
    def test(self,visualization=False):
        ...
        for src_id, trg_id in self.dataset_configs.scenarios:
            ...
            if visualization == True:
                self.visualize()
                dic = {'src_all_features':self.src_all_features,'src_true_labels':self.src_true_labels,
                           'trg_all_features':self.trg_all_features,'trg_true_labels':self.trg_true_labels,
                           'trg_test_all_features':self.trg_test_all_features,'trg_test_true_labels':self.trg_test_true_labels,
                            'src_pred_labels':self.src_pred_labels,
                       'trg_pred_labels':self.trg_pred_labels,'trg_test_pred_labels':self.trg_test_pred_labels,
                            'best_acc':best_results.iloc[-1]['acc'],'best_f1':best_results.iloc[-1]['f1_score'],'best_auroc':best_results.iloc[-1]['auroc']}
                visualization_file_name = "saved_dictionary_"+src_id + "_to_" + trg_id + ".pickle"
                with open(self.scenario_log_dir+"/"+visualization_file_name, 'wb') as handle:
