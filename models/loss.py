...
class VAT(nn.Module):
    def __init__(self, feature_extractor,classifier, configs, device):
    ...
    def generate_virtual_adversarial_perturbation(self, x, logit):
        ...
        for _ in range(self.n_power):
            ...
            if self.backbone=="tf_encoder": x_feat, _ = self.feature_extractor(x + d)
            else: x_feat = self.feature_extractor(x + d)
                
            logit_m = self.classifier(x_feat)
    ...
    def virtual_adversarial_loss(self, x, logit):
        ...
        if self.backbone=="tf_encoder": x_feat, _ = self.feature_extractor(x + r_vadv)
        else: x_feat = self.feature_extractor(x + r_vadv)
...
class SinkhornDistance(nn.Module):
    ...
    """
    def __init__(self, eps, max_iter, reduction='none',device=None):
    ...
    def forward(self, x, y):
        ...
        C = self._cost_matrix(x, y).to(self.device)  # Wasserstein cost function
        x_points = x.shape[-2]
        y_points = y.shape[-2]
        if x.dim() == 2:
            batch_size = 1
    ...
    def M(self, C, u, v):
        ...
        return ((-C + u.unsqueeze(-1) + v.unsqueeze(-2)) / self.eps).to(self.device)
