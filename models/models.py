...
class tf_encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if ef.dim()==1: ef = ef.reshape(-1,ef.shape[0])
...
class CNN_Decoder(nn.Module):
    def __init__(self, configs):
        ...
        self.conv_transpose_block1 = nn.Sequential(
            nn.ConvTranspose1d(configs.final_out_channels, configs.mid_channels * 2, kernel_size=8, stride=2, padding=1, output_padding=1),
            nn.BatchNorm1d(configs.mid_channels * 2),
            nn.ReLU()
        )

        self.conv_transpose_block2 = nn.Sequential(
            nn.ConvTranspose1d(configs.mid_channels * 2, configs.mid_channels, kernel_size=8, stride=2, padding=1, output_padding=1),
            nn.BatchNorm1d(configs.mid_channels),
            nn.ReLU()
        )

        self.conv_transpose_block3 = nn.Sequential(
            nn.ConvTranspose1d(configs.mid_channels, configs.input_channels, configs.sequence_len),
            nn.BatchNorm1d(configs.input_channels) #, nn.Sigmoid() 
        )
...
class classifier(nn.Module):
    def __init__(self, configs):
        ...
        if self.configs.backbone!="tf_encoder": 
            self.logits = nn.Linear(configs.features_len * configs.final_out_channels, configs.num_classes)
        else:
            self.logits = nn.Linear(configs.features_len * configs.out_dim, configs.num_classes)
...
class TCN_Decoder(nn.Module):
    def __init__(self, configs):
        ...
        # Adjust these to match the encoder output and expand correctly
        ...
        self.deconv_block1 = nn.Sequential(
            nn.ConvTranspose1d(out_channels1, in_channels1, kernel_size, stride=stride1, padding=padding1, dilation=dilation1),
            nn.ReLU(),
            nn.ConvTranspose1d(in_channels1, in_channels1, kernel_size, stride=stride1, padding=padding1, dilation=dilation1),
            nn.ReLU(),
        )

        # Adjust the output of the last layer to match the original input size
        out_channels0 = configs.input_channels
        in_channels0 = configs.tcn_layers[0]
        dilation0 = 1
        padding0 = (kernel_size - 1) * dilation0 // 2

        self.deconv_block2 = nn.Sequential(
            nn.ConvTranspose1d(in_channels1, in_channels0, kernel_size, stride=stride1, padding=padding0, dilation=dilation0),
            nn.ReLU(),
            nn.ConvTranspose1d(in_channels0, out_channels0, kernel_size, stride=stride1, padding=padding0, dilation=dilation0),
        )
...
class codats_classifier(nn.Module):
    def __init__(self, configs):
        ...
        self.configs = configs
        if self.configs.backbone=="tf_encoder": 
            final_out_channels = configs.out_dim
        else:
            final_out_channels = configs.out_dim
        ...
        self.logits = nn.Sequential(
            nn.Linear(model_output_dim * final_out_channels, self.hidden_dim),
            nn.ReLU(),
...
class Discriminator_CDAN(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.layer = nn.Sequential(
            nn.Linear(configs.features_len * final_out_channels * configs.num_classes, configs.disc_hid_dim),
            nn.ReLU(),
            nn.Linear(configs.disc_hid_dim, configs.disc_hid_dim),
            nn.ReLU(),
            nn.Linear(configs.disc_hid_dim, 2)
...
class AdvSKM_Disc(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.input_dim = configs.features_len * final_out_channels
